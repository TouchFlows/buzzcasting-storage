...
**Why this exists**: before this, the same local `release` script ended with `npm publish` itself - a version could be bumped/tagged/committed locally without the publish step actually completing (or without ever running `npm run release` at all after a hand-edited version bump), and nothing would notice. That happened for real once (`buzzcasting-utils` hand-bumped for `EVENTS.UNSUBSCRIBE` but never published - only surfaced as a confusing type error in a downstream consumer's CI). Moving the actual `npm publish` into CI, gated on a real tag push, means "the tag exists on GitHub" and "the version is live on npm" can no longer silently drift apart.
...
- Repository: `buzzcasting-storage`
...
- `.github/workflows/main.yml` still runs build+test on every push/PR, independent of releases.
