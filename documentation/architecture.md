...
### ✅ A second, unrelated `subscribers` in `buzzcasting-app` — removed

`buzzcasting-app.tsx` used to keep its own `window.__bc.subscribers` (a `Map`, not this package's `subscribers` object), populated by a plain DOM `CustomEvent(EVENTS.SUBSCRIBE)` that some (not all) inner components emit directly — a completely separate path from the `Widget`/`BroadcastChannel` mechanism documented above. It had the identical never-pruned-on-unmount problem this fix addresses, but its only consumer, `window.__bc.api.worker()`, was itself dead code (never called anywhere) — so it was an inert, session-long-growing `Map` with no live effect, not the cause of the growing-fetch-list symptom the fix above was actually chasing. Since it was fully unreachable, it (and the dead `worker()` function, and the `subscriberAdded`/`getSubscribers`/`deleteSubscribers` members on `buzzcasting-app.tsx`) have been removed outright rather than fixed — there was nothing worth keeping. The inner components that used to emit into it (`series-kpi`, `cloud-barchart`, etc.) still call `this.subscribe.emit()`, now a harmless no-op with nothing listening; not worth touching those ~8 files just to delete an inert emit call.
