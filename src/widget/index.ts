import type { IModal, IQuery, IResponse } from 'buzzcasting-utils'
import { API, attrs, clearContents, CSS, EVENTS, log } from 'buzzcasting-utils'
...
export default class Widget {
  ...
  constructor(
    ...
    query.presentation
			= window.BuzzCasting.getOptions()?.presentation ?? `${selector} not found` // = element.closest<any>(selector)?.getAttribute('presentation')
    ...
    // @ts-expect-error query may be undefined but dhould not be
      ...
      switch (messageEvent.data.event) {
        case EVENTS.WIDGET_UPDATE:
          try {
            ...
            log(4, [EVENTS.WIDGET_UPDATE, update, error.message])
          }
          break
